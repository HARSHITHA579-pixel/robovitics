...
export default function Events() {
  ...
  useEffect(() => {
    ...
    // UPDATED: Brought the toggle button animation way closer to the end of the card animation (from 0.96 down to 0.45)
    if (toggleRef.current) {
      tl.to(toggleRef.current, { opacity: 1, y: 0, pointerEvents: "auto", duration: 0.12, ease: "power2.out" }, 0.45);
    }

    stRef.current = ScrollTrigger.create({
      trigger: section,
      start: "top top",
      // UPDATED: Reduced scroll distance significantly from innerHeight * 5 down to innerHeight * 2. 
      // This strips out all the extra "dead" scrolling.
